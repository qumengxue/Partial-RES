...
def evaluate_model(epoch,
    ...
    with torch.no_grad():
        for batch, inputs in enumerate(loader):
            ...
            content.extend(list(zip(batch_mask_iou.int().tolist(), gt_mask, pred_masks)))
    ...
    if loader.dataset.which_set=='train':
        if not os.path.exists('./refine_data/'):
            os.makedirs('./refine_data/')
        torch.save(content, "./refine_data/{}_instances_refine.pth".format(cfg.filename.split('_')[-1].split('.')[0]))
        assert False
